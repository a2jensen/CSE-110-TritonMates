...
import {getAllTasks, deleteTask, updateTask} from "../../../app/api/tasks/TaskContext";
...
const changeTask = async(roomID: string, taskID: string, name: string, points: number, assignee: string, status:string, due_date: string) => {
  await updateTask(roomID,taskID, name, points, assignee ,status,  new Date(due_date));

}


const fetchTasks = async(roomID: string, tasks:Task[], setTasks: React.Dispatch<React.SetStateAction<Task[]>>) => {
  ...
  const toggleTask = (id: string) => {
    ...
    for (let i = 0; i < tasks.length; i++) {
        if (tasks[i].id == id){
           console.log(tasks[i])
           if (tasks[i].done){
             changeTask(roomID, id, tasks[i].text,tasks[i].points, tasks[i].assignee,'in progress', tasks[i].dueDate)

           }
           else{
             changeTask(roomID, id, tasks[i].text,tasks[i].points, tasks[i].assignee,'done', tasks[i].dueDate)
